...
//export class LinqIterable<T> {
    // ... still no implementation ...
//}

declare global {
    interface Array<T> {
        asLinq(): Linq.LinqIterableBase<T>;
    }

    interface String {
        asLinq(): Linq.LinqIterableBase<string>;
    }
    ...
    interface Map<K, V> {
        asLinq(): Linq.LinqIterableBase<Linq.KeyValuePair<K, V>>;
    }
...
Array.prototype.asLinq = function () {
    return new Linq.LinqIterableProxy(this);
}
...
Map.prototype.asLinq = function() {
    return (new Linq.LinqIterableProxy(this)).select(x => new Linq.KeyValuePair(x[0], x[1]));
}

Object.prototype.asLinq = function() {
    let that = this;
    return Object.keys(this).asLinq().select(k => new Linq.KeyValuePair(k, (<any>that)[k]));
}
